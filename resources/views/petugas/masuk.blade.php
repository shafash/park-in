...
@section('page-sub','Daftarkan kendaraan masuk area parkir')

@section('topbar-right')
<a href="{{ route('petugas.transaksi.index') }}" class="btn btn-out btn-sm">← Transaksi</a>
@endsection
...
<div class="two-col">
  ...
  <div class="panel">
    ...
    <div class="pb-body">
      <form method="POST" action="{{ route('petugas.transaksi.masuk.store') }}" id="form-masuk">
        ...
        {{-- ── INFO KENDARAAN (tampil jika dipilih dari DB) ─ --}}
          ...
          <select name="id_tarif" id="sel_tarif" required>
            ...
            @foreach($tarifs as $t)
              <option value="{{ $t->id_tarif }}"
                ...
                {{ $t->jenis_kendaraan === 'lainnya' ? 'Truk' : ucfirst($t->jenis_kendaraan) }} — {{ $t->rupiah }}/jam
              </option>
            @endforeach
          </select>
        </div>
        ...
        <div class="fg">
          ...
          <input type="text" id="waktu_masuk_display" value="{{ now()->format('d/m/Y H:i:s') }}"
  ...
  <div class="panel">
    ...
    @php $pct=$a->okupansi; $col=$a->okupansiColor; @endphp
    <div style="padding:14px 22px;border-bottom:1px solid var(--bdr)">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px">
        <span class="fw7">{{ $a->nama_area }}</span>
      ...
      <div style="font-size:11px;color:var(--gray2);margin-top:4px">{{ $a->terisi }}/{{ $a->kapasitas }} terisi</div>
    </div>
    @endforeach
  </div>
</div>
